...
from oasis.minibox import load_objects, load_seeds
...
def classify_single_mini_box(
    ...
    # Load particles
    pos_part, vel_part, pid, mass_part = \
        load_objects(mini_box_id, boxsize, minisize, load_path, padding, name=name)

    # Load calibration parameters
    with h5.File(load_path + f'calibration_pars_{name}.hdf5', 'r') as hdf:
    ...
    # cache KDTree queries for all seeds
    ...
    for i in tqdm(range(n_seeds), ncols=100, desc='Finding haloes', colour='green', disable=disable_tqdm):
        ...
        # Check if enough orbiting particles found
        ...
        # Classify seeds
        # ------------------------------
        rel_pos_seed = rel_pos_seed_all[i]  # rel pos of other seeds to seed i
        r_max = 2.0 * r200b_i
        ...
        if n_seeds_near > 0:
            ...
            # Precompute delta_pos / delta_vel / rp_sq / vp_sq
            ...
            if fast_mass:
                ...
                while is_halo and (j < n_seeds_near):
                    # Directly use precomputed delta_pos, delta_vel
                    rp_sq = rp_sq_all[:, j]
                    vp_sq = vp_sq_all[:, j]
                    r_ij = r_ij_all[j]
        ...
        orb_mass.append(mass_part[idx][mask_orb])
        orb_hid.append(orb_seed)

       
  
    orb_pid = np.concatenate(orb_pid)
    orb_mass = np.concatenate(orb_mass)
    ...
    for i in tqdm(range(len(haloes)), ncols=100, desc='Saving particles (fast)',
        ...
        mass_range = orb_mass[lidx:ridx]
        new_orb_set = set(new_orb)
        pid_range = np.array(pid_range)
        mass_range = np.array(mass_range)
        mask = np.isin(pid_range, list(new_orb_set))
        new_mass = mass_range[mask]
...
def run_orbiting_mass_assignment(
    ...
    func = partial(classify_single_mini_box, min_num_part=min_num_part,
                   ...
                   disable_tqdm=True, seed_range=seed_range, name=name)
    ...
    for i in tqdm(range(n_mini_boxes), ncols=100, desc='Merging catalogues',
                  colour='green'):
        if not os.path.exists(save_path + f'{i}.hdf5'):
            continue
